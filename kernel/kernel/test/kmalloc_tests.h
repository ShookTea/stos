...
static inline bool kmalloc_test_basic_small(void) {
    ...
    if (!ptr8 || !ptr16 || !ptr32 || !ptr64) {
        debug_printf("  FAILED: Could not allocate small sizes\n");
        kfree(ptr8);
        kfree(ptr16);
        kfree(ptr32);
        return false;
    ...
    debug_printf("  Allocated 8 bytes at:  %#x\n", (uint32_t)ptr8);
    debug_printf("  Allocated 16 bytes at: %#x\n", (uint32_t)ptr16);
    debug_printf("  Allocated 32 bytes at: %#x\n", (uint32_t)ptr32);
    debug_printf("  Allocated 64 bytes at: %#x\n", (uint32_t)ptr64);
    ...
    *(uint64_t*)ptr64 = 0xDDDDDDDDDDDDDDDDULL;
    ...
    if (!verify) {
        debug_printf("  FAILED: Data verification failed\n");
        kfree(ptr8);
        kfree(ptr16);
        kfree(ptr32);
        kfree(ptr64);
    ...
    debug_printf("  All allocations freed\n");
    debug_printf("  PASSED\n");
    return true;
}
...
static inline bool kmalloc_test_large(void) {
    ...
    if (!ptr4k || !ptr8k || !ptr16k) {
        debug_printf("  FAILED: Could not allocate large sizes\n");
        kfree(ptr4k);
        kfree(ptr8k);
        return false;
    }

    debug_printf("  Allocated 4KB at:  %#x\n", (uint32_t)ptr4k);
    debug_printf("  Allocated 8KB at:  %#x\n", (uint32_t)ptr8k);
...
static inline bool kmalloc_test_calloc(void) {
    ...
    if (!ptr) {
        debug_printf("  FAILED: Could not allocate with kcalloc\n");
        return false;
    }

    debug_printf("  Allocated 400 bytes (100 x 4) at: %#x\n", (uint32_t)ptr);
    ...
    for (int i = 0; i < 400; i++) {
        if (bytes[i] != 0) {
            debug_printf("  FAILED: Byte %d is not zero (value: %#x)\n", i, bytes[i]);
            all_zero = false;
            break;
        }
    }
    ...
    debug_printf("  Verified all 400 bytes are zero\n");
...
static inline bool kmalloc_test_realloc(void) {
    ...
    void* ptr = kmalloc(64);
    if (!ptr) {
        debug_printf("  FAILED: Initial allocation failed\n");
        return false;
    }

    debug_printf("  Initial allocation: 64 bytes at %#x\n", (uint32_t)ptr);
    ...
    if (!ptr2) {
        debug_printf("  FAILED: Reallocation to 128 bytes failed\n");
        kfree(ptr);
        return false;
    }

    debug_printf("  Reallocated to 128 bytes at %#x\n", (uint32_t)ptr2);

    // Verify old data is preserved
    bool data_preserved = true;
    for (int i = 0; i < 64; i++) {
        if (((uint8_t*)ptr2)[i] != (i & 0xFF)) {
            debug_printf("  FAILED: Data not preserved at byte %d\n", i);
    ...
    debug_printf("  Data preserved after reallocation\n");

    // Reallocate to smaller size
    void* ptr3 = krealloc(ptr2, 32);
    if (!ptr3) {
        debug_printf("  FAILED: Reallocation to 32 bytes failed\n");
        kfree(ptr2);
        return false;
    }

    debug_printf("  Reallocated to 32 bytes at %#x\n", (uint32_t)ptr3);

    // Verify first 32 bytes are still intact
    for (int i = 0; i < 32; i++) {
        if (((uint8_t*)ptr3)[i] != (i & 0xFF)) {
            debug_printf("  FAILED: Data not preserved after shrinking\n");
            kfree(ptr3);
            return false;
        }
    }

    debug_printf("  Data preserved after shrinking\n");
...
static inline bool kmalloc_test_strdup(void) {
    ...
    if (!duplicate) {
        debug_printf("  FAILED: kstrdup returned NULL\n");
        return false;
    }

    debug_printf("  Original string: '%s' at %#x\n", original, (uint32_t)original);
    debug_printf("  Duplicated string: '%s' at %#x\n", duplicate, (uint32_t)duplicate);

    // Verify strings match
    if (strcmp(original, duplicate) != 0) {
        debug_printf("  FAILED: Strings don't match\n");
        kfree(duplicate);
        return false;
    }

    debug_printf("  Strings match\n");

    // Verify they're at different addresses
    if (original == duplicate) {
        debug_printf("  FAILED: Duplicate points to same address\n");
        kfree(duplicate);
        return false;
    }

    debug_printf("  Strings are at different addresses (actual copy)\n");

    // Test kstrndup
    char* partial = kstrndup(original, 5);
    if (!partial) {
        debug_printf("  FAILED: kstrndup returned NULL\n");
        kfree(duplicate);
        return false;
    }

    debug_printf("  Partial copy (5 chars): '%s'\n", partial);

    if (strcmp(partial, "Hello") != 0) {
        debug_printf("  FAILED: kstrndup didn't copy correctly\n");
        kfree(duplicate);
        kfree(partial);
        return false;
    }
...
static inline bool kmalloc_test_multiple(void) {
    ...
    for (int i = 0; i < NUM_ALLOCS; i++) {
        ...
        if (!ptrs[i]) {
            debug_printf("  FAILED: Allocation %d failed (size %zu)\n", i, size);
    ...
    debug_printf("  Successfully allocated %d blocks\n", NUM_ALLOCS);
    ...
    for (int i = 0; i < NUM_ALLOCS; i++) {
        ...
        for (size_t j = 0; j < size; j++) {
            if (bytes[j] != (uint8_t)(i & 0xFF)) {
                debug_printf("  FAILED: Data corruption in block %d at byte %zu\n", i, j);
                verify = false;
                break;
            }
        }
    ...
    debug_printf("  All data verified\n");
...
static inline bool kmalloc_test_stress_small(void) {
    ...
    for (int i = 0; i < NUM_ALLOCS; i++) {
        ...
        if (!ptrs[i]) {
            debug_printf("  FAILED: Allocation %d failed\n", i);
            for (int j = 0; j < i; j++) {
                kfree(ptrs[j]);
            }
            return false;
    ...
    debug_printf("  Successfully allocated %d small blocks (32 bytes each)\n", NUM_ALLOCS);

    // Verify all values
    bool verify = true;
    for (int i = 0; i < NUM_ALLOCS; i++) {
        if (*(uint32_t*)ptrs[i] != (uint32_t)i) {
            debug_printf("  FAILED: Value mismatch at block %d\n", i);
            verify = false;
            break;
        }
    }
    ...
    debug_printf("  All values verified\n");
    ...
    debug_printf("  Freed half the blocks\n");

    // Allocate new blocks
    for (int i = 0; i < NUM_ALLOCS / 2; i++) {
        ptrs[i] = kmalloc(32);
        if (!ptrs[i]) {
            debug_printf("  FAILED: Re-allocation %d failed\n", i);
    ...
    debug_printf("  Re-allocated freed blocks\n");
...
static inline bool kmalloc_test_edge_cases(void) {
    ...
    kfree(NULL);
    debug_printf("  kfree(NULL) handled correctly\n");

    // Test zero-size allocation
    void* ptr_zero = kmalloc(0);
    if (ptr_zero != NULL) {
        debug_printf("  WARNING: kmalloc(0) returned non-NULL: %#x\n", (uint32_t)ptr_zero);
        kfree(ptr_zero);
    } else {
        debug_printf("  kmalloc(0) returned NULL (expected)\n");
    }

    // Test very small allocation
    void* ptr1 = kmalloc(1);
    if (!ptr1) {
        debug_printf("  FAILED: Could not allocate 1 byte\n");
        return false;
    }
    *(uint8_t*)ptr1 = 0x42;
    if (*(uint8_t*)ptr1 != 0x42) {
        debug_printf("  FAILED: 1-byte allocation corrupted\n");
        kfree(ptr1);
        return false;
    }
    debug_printf("  1-byte allocation works correctly\n");
    kfree(ptr1);
    ...
    if (!ptr_thresh) {
        debug_printf("  FAILED: Could not allocate 1024 bytes (largest slab size)\n");
        return false;
    }
    debug_printf("  Allocation at largest slab size (1024 bytes) works\n");
    kfree(ptr_thresh);

    // Test allocation just above slab threshold (should use large allocator)
    void* ptr_above = kmalloc(KMALLOC_SLAB_THRESHOLD + 1);
    if (!ptr_above) {
        debug_printf("  FAILED: Could not allocate above slab threshold\n");
        return false;
    }
    debug_printf("  Allocation above slab threshold (%d bytes) uses large allocator\n", KMALLOC_SLAB_THRESHOLD + 1);
    kfree(ptr_above);

    // Test krealloc with NULL (should behave like kmalloc)
    void* ptr_realloc = krealloc(NULL, 64);
    if (!ptr_realloc) {
        debug_printf("  FAILED: krealloc(NULL, 64) failed\n");
        return false;
    }
    debug_printf("  krealloc(NULL, size) works like kmalloc\n");
    kfree(ptr_realloc);
    ...
    if (result != NULL) {
        debug_printf("  WARNING: krealloc(ptr, 0) returned non-NULL\n");
        kfree(result);
    } else {
        debug_printf("  krealloc(ptr, 0) works like kfree\n");
    }
...
static inline bool kmalloc_test_size_tracking(void) {
    ...
    for (int i = 0; i < num_allocs; i++) {
        ...
        if (!allocs[i].ptr) {
            debug_printf("  FAILED: Could not allocate %zu bytes\n", allocs[i].requested);
            for (int j = 0; j < i; j++) {
                kfree(allocs[j].ptr);
            }
            return false;
    ...
    debug_printf("  Allocated %d blocks of various sizes\n", num_allocs);
    ...
    for (int i = 0; i < num_allocs; i++) {
        ...
        if (reported_size < allocs[i].requested) {
            debug_printf("  FAILED: kmalloc_size(%zu) returned %zu (too small)\n",
                   allocs[i].requested, reported_size);
            verify = false;
            break;
        }

        debug_printf("  Requested %zu bytes, kmalloc_size reports %zu bytes\n",
               allocs[i].requested, reported_size);
    }
...
static inline bool kmalloc_test_macros(void) {
    ...
    if (!s1) {
        debug_printf("  FAILED: kmalloc_struct failed\n");
        return false;
    }
    s1->a = 1;
    s1->b = 2;
    s1->c = 3;
    debug_printf("  kmalloc_struct works: allocated test_struct_t at %#x\n", (uint32_t)s1);

    // Test kmalloc_array
    uint32_t* arr = kmalloc_array(uint32_t, 10);
    if (!arr) {
        debug_printf("  FAILED: kmalloc_array failed\n");
        kfree(s1);
        return false;
    }
    for (int i = 0; i < 10; i++) {
    ...
    debug_printf("  kmalloc_array works: allocated array of 10 uint32_t at %#x\n", (uint32_t)arr);

    // Test kcalloc_struct
    test_struct_t* s2 = kcalloc_struct(test_struct_t);
    if (!s2) {
        debug_printf("  FAILED: kcalloc_struct failed\n");
        kfree(s1);
        kfree(arr);
        return false;
    }
    if (s2->a != 0 || s2->b != 0 || s2->c != 0) {
        debug_printf("  FAILED: kcalloc_struct not zero-initialized\n");
        kfree(s1);
        kfree(arr);
        kfree(s2);
        return false;
    }
    debug_printf("  kcalloc_struct works: zero-initialized struct at %#x\n", (uint32_t)s2);

    // Test kcalloc_array
    uint32_t* arr2 = kcalloc_array(uint32_t, 10);
    if (!arr2) {
        debug_printf("  FAILED: kcalloc_array failed\n");
        kfree(s1);
        kfree(arr);
        kfree(s2);
        return false;
    ...
    if (!all_zero) {
        debug_printf("  FAILED: kcalloc_array not zero-initialized\n");
        kfree(s1);
        kfree(arr);
        kfree(s2);
        kfree(arr2);
    ...
    debug_printf("  kcalloc_array works: zero-initialized array at %#x\n", (uint32_t)arr2);
...
static inline bool kmalloc_test_double_free(void) {
    ...
    if (!ptr_small) {
        debug_printf("  FAILED: Could not allocate small block\n");
        return false;
    }
    debug_printf("  Allocated small block (64 bytes) at %#x\n", (uint32_t)ptr_small);
    ...
    kfree(ptr_small);
    debug_printf("  First kfree() succeeded\n");
    ...
    debug_printf("  Attempting second kfree() on same pointer...\n");
    kfree(ptr_small);
    ...
    if (stats_after_second.num_small_frees != stats_after_first.num_small_frees) {
        debug_printf("  FAILED: Stats were updated on double-free (small allocation)\n");
        debug_printf("    Frees before: %zu, after: %zu\n",
               stats_after_first.num_small_frees,
               stats_after_second.num_small_frees);
        return false;
    }
    debug_printf("  Small allocation double-free correctly prevented\n");
    ...
    debug_printf("  (Large allocation double-free test skipped - causes page fault)\n");
...
static inline void kmalloc_run_all_tests(void) {
    debug_printf("\n========================================\n");
    debug_printf("     kmalloc/kfree Test Suite\n");
    debug_printf("========================================\n");
